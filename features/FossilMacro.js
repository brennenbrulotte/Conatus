...
class FossilMacro {
    constructor() {
        ...
        this.key = new brennenKeyBind("Fossil Macro", Keyboard.KEY_NONE, "Conatus")
        this.lastGuiOpened;
        this.fossils = [
          `00100
           01010
        ...
        this.key.registerKeyPress(() => {
            ...
            if(this.enabled) {
                brennenNotification.push("Brennen", "Fossil Macro: Enabled")
                this.startThread(this.threadStart);
            } else {
              this.startThread(new Thread(() => {}))
              brennenNotification.push("Brennen", "Fossil Macro: Disabled")
            }
        })

        this.chatRegister = brennenRegister("chat", () => {
          if(!this.enabled) return;
          this.enabled = !this.enabled;
          ChatLib.chat(`${brennenPrefix}No more scrap :(`)
        }).setCriteria("You need to put both a chisel and a fossil in the machine!")

        this.guiTest = brennenRegister('guiKey', (char, keycode, gui, event) => {
          if(!this.enabled) return;
          if(keycode !== this.key.getKeyCode()) return;
          this.enabled = !this.enabled

          if(this.enabled) {
            ChatLib.chat(`${brennenPrefix}Enabled.`)
            this.startThread(this.threadStart);
          } else {
            this.startThread(new Thread(() => {}))
            ChatLib.chat(`${brennenPrefix}Disabled.`)
          }
        }).unregister();

        brennenRegister("guiOpened", (event) => {
          if(!this.enabled) return;
          this.guiTest.register()
          this.lastGuiOpened = event.gui
        })

        brennenRegister("guiClosed", () => {
          if(!this.enabled) return;
    ...
    threadStart = new Thread(() => {
        ...
        if(!this.chiselIndex || this.chiselIndex == -1 || !this.scrapIndex || this.scrapIndex == -1) {
          ChatLib.chat(`${brennenPrefix}Scrap or Chisel Missing.`);
          this.enabled = !this.enabled
          return;
        } else {
          Thread.sleep(this.delay)
  ...
  threadNot = new Thread(() => {
      if(!this.enabled) return;
        ...
        if(this.getCharges(this.lastGuiOpened) == 0) {
            ...
            ChatLib.chat(`${brennenPrefix}Restarting...`)
    ...
    threadFound = new Thread(() => {
        ...
        if(slot == -1 || !slot) {
            ...
            if(retrySlot == -1 || !retrySlot) {
                this.startThread(new Thread((() => {})));
                ChatLib.chat(`${brennenPrefix}Slot Bug. Restarting...`);
                if(this.restarting) return;
                ChatLib.chat(`${brennenPrefix}Restarting...`)
                this.restarting = true;
                Client.currentGui.close();
